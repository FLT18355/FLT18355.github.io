...
### 3.2 字体门控(site-font)
- `font.woff2` 7.1MB,`@font-face` 声明不触发下载,只有 `html[data-font="maple"]` 规则下的元素实际引用才下载。
- **改字体相关 CSS 时,不要**把 "Maple Mono NF CN" 写进无条件生效的规则(如默认 `body` 或 `:root` 的 `--mono`)。必须挂在 `html[data-font="maple"]` 下,否则弱网用户会被迫下载 7.1MB。
